...
function FilterByTeamxSortByNoOfAwards(team){
  return players.filter((player) => player.team == team).sort((player1, player2) => player2.awards.length - player1.awards.length);
}

//Challenge 1 - Sort players that have won _______ award _____ times and belong to _______ country in alphabetical order of their names
function SortByNamexAwardxTimes(awardName, noOfTimes, country){
  return players.filter((player) => player.awards.filter((award) => award.name == awardName).length == noOfTimes && player.country == country).sort((player1, player2) => player1.name.localeCompare(player2.name));
}

//Challenge 2 - Sort players that are older than _____ years in alphabetical order
//Sort the awards won by them in reverse chronological order
function sortByNamexOlderThan(age) {
  return players.filter((player) => player.age > age).sort((player1, player2) => {
      const awardsA = player1.awards.map((award) => award.year).sort().reverse();
      const awardsB = player2.awards.map((award) => award.year).sort().reverse();
